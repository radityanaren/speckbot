"""Interactive onboarding questionnaire for speckbot."""
...
from speckbot.cli.model_info import (
    format_token_count,
...
def _show_config_panel(display_name: str, model: BaseModel, fields: list) -> None:
    ...
    console.print(Panel(table, title=f"[bold]{display_name}[/bold]", border_style="#ff6b6b"))
...
def _show_main_menu_header() -> None:
    ...
    console.print(Align.center(f"{__logo__} [bold cyan]SpeckBot[{__version__}][/bold cyan]"))
    console.print()
...
def _show_section_header(title: str, subtitle: str = "") -> None:
    ...
    if subtitle:
        console.print(
            Panel(f"[dim]{subtitle}[/dim]", title=f"[bold]{title}[/bold]", border_style="#ff6b6b")
        )
    else:
        console.print(Panel("", title=f"[bold]{title}[/bold]", border_style="#ff6b6b"))
...
def _try_auto_fill_context_window(model: BaseModel, new_model_name: str) -> None:
    ...
    Note:
        This function imports AgentDefaults from speckbot.config.schema to get
    ...
    from speckbot.config.schema import AgentDefaults
...
def _get_channel_info() -> dict[str, tuple[str, type[BaseModel]]]:
    ...
    for name, channel_cls in discover_all().items():
        try:
            mod = importlib.import_module(f"speckbot.channels.{name}")
...
def _print_summary_panel(rows: list[tuple[str, str]], title: str) -> None:
    ...
    console.print(Panel(table, title=f"[bold]{title}[/bold]", border_style="#ff6b6b"))
    ...
    try:
        ...
        for name, display in _get_provider_names().items():
            try:
                ...
                logger.debug("Error reading provider {}: {}", name, e)
                continue
        ...
        for name, display in _get_channel_names().items():
            try:
                ...
                logger.debug("Error reading channel {}: {}", name, e)
                continue
        ...
        for title, model in [
            ...
            try:
                ...
                logger.debug("Error summarizing {}: {}", title, e)
                continue
        ...
        logger.warning("Error displaying configuration summary: {}", e)
